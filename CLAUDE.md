...
**수집 범위**: 14개 지역(대도시 6곳 도쿄/오사카/후쿠오카/나고야/삿포로/오키나와는 `maxResults: 20`, 소도시 8곳 다카마쓰/마쓰야마/오카야마/히로시마/시즈오카/요나고/기타큐슈/나가사키는 `maxResults: 15`) × 6개 카테고리(스시/라멘/우동/오코노미야키/돈카츠/타코야키) = 84개 조합. 이 84개 조합은 매번 Python 스크립트로 순회 호출해서 수집함(스크립트 파일은 저장 안 해두고 대화 중 임시로 작성) — 재수집이 필요하면 이 조합 리스트를 그대로 재사용할 것.
...
- `SearchResultGridCard.jsx`의 썸네일에는 **사진 캐러셀**이 있음 — 가게당 사진이 여러 장(`restaurant.images`, 최대 5장)이라 hover 시 좌우 화살표(`IconChevronRight`, 왼쪽은 `rotate-180`)와 하단 점 인디케이터로 넘겨볼 수 있음. 화살표 클릭은 `stopPropagation`으로 카드 클릭(상세 이동)과 분리됨. Google이 사진에 "음식 사진"/"가게 사진" 같은 카테고리 라벨을 안 줘서, 순서는 Google이 반환한 그대로이고 음식/외관을 앞뒤로 재배열하는 로직은 없음.
...
- **구글 리뷰 카드는 클릭 가능한 링크임**: `ReviewCard.jsx`에서 `source !== 'naver'`(구글 리뷰)일 때만 카드 전체가 `<a target="_blank">`가 되어 그 가게의 Google Maps 페이지로 이동함. 네이버 출처 리뷰(mock 잔재)는 실제 링크가 없어 클릭 불가 — `cursor-pointer` 스타일도 안 붙음. 클릭 가능한 카드 하단에는 "클릭하면 구글맵 가게 페이지로 이동해요 — 상단 '리뷰' 탭을 눌러 전체 리뷰를 확인하세요" 안내 문구가 있음(위 리뷰 탭 직행 불가 이슈에 대한 보완).
