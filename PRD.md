...
**UI 노출 완료**(2026-08-18) — `RestaurantDetailPage.jsx`의 주소·전화번호 블록 바로 아래에 영업시간(정휴일 포함)·예산대(디너/런치)·핫페퍼 그루메 링크를 추가로 표시. 값이 있는 필드만 조건부 렌더링(매칭 안 된 가게는 항목 자체가 안 보임), 원문(일본어)을 별도 번역 없이 그대로 노출.
