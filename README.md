...
| `enrich-hotpepper` | 리쿠르트 Hot Pepper Gourmet API로 영업시간/정휴일/예산대/카드결제/핫페퍼 URL을 보강(2026-08-18, 전체 1,970곳 실행 — 1,589곳 매칭; 2026-08-20에 신규 카테고리 1,794곳 추가 실행 — 1,567곳 매칭). 가게명 대신 위경도 거리(80m 이내) 기준으로 매칭. `skipTranslate` 옵션으로 Cloud Translation API 호출 없이 일본어 원문만 저장하는 모드도 지원(신규 카테고리 보강 시 크레딧 절약 목적으로 사용, 해당분은 아직 미번역) |
...
- ~~핫페퍼 그루메 API 보강~~: 2026-08-18 완료 — `enrich-hotpepper`로 영업시간/정휴일/예산대/카드결제/URL을 보강(1,970곳 중 1,589곳 매칭), 상세페이지 사이드바에도 노출 완료. 2026-08-20에 신규 카테고리 1,794곳도 추가 보강(1,567곳 매칭, 번역 없이 일본어 원문만 저장 — 크레딧 여유 생기면 번역 백필 필요)
