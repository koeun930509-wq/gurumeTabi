...
| `enrich-hotpepper` | 리쿠르트 Hot Pepper Gourmet API로 영업시간/정휴일/예산대/카드결제/핫페퍼 URL을 보강(2026-08-18, 전체 1,970곳 실행 — 1,589곳 매칭). 가게명 대신 위경도 거리(80m 이내) 기준으로 매칭 |
...
수집 범위는 14개 지역(도쿄/오사카/후쿠오카/나고야/삿포로/오키나와/다카마쓰/마쓰야마/오카야마/히로시마/시즈오카/요나고/기타큐슈/나가사키) × 7개 카테고리(스시/라멘/우동/오코노미야키/돈카츠/타코야키/디저트)이며, **분기(3개월) 1회 재수집을 권장합니다**(2026-08-19부터, 기존 월 1회에서 변경 — 전체 재수집 1회당 Google Places + Translation API 비용이 약 9~10만 원 실측되어 무료 크레딧 소진을 늦추기 위함, 자세한 절차는 `CLAUDE.md` 참고). "카페"는 필터 목록엔 있지만 아직 미수집.
...
| `/place/:id` | 맛집 상세 — 히어로 이미지(사진 없으면 `defaultHero.png` 대체) 정중앙에 가게 이름 표시, 좌상단 원형 뒤로가기 아이콘 버튼(텍스트 없이 화살표만). 콘텐츠 시트에 평점·현지인 비율·영업 상태 배지 + 해시태그(모바일은 좌우 padding을 PC의 절반으로 축소). 위치 지도는 **Google Maps Embed API**로 실제 표시되고, "구글 지도" 버튼은 `place_id` 기반으로 정확한 가게 페이지로 연결됨(단, "리뷰" 탭까지 자동 진입은 안 되고 개요 탭에서 열림 — Places API가 리뷰탭 직행에 필요한 CID를 안 줌). 오른쪽 사이드바는 가게정보(주소·전화 아래에 핫페퍼로 보강한 영업시간/정휴일/예산대/핫페퍼 링크가 있으면 함께 표시, 2026-08-18 추가 — 영업시간·정휴일·예산 텍스트는 Cloud Translation API로 한글 번역해서 저장됨) → 지도 → ⭐저장하기/근처 디저트 맛집 버튼(나란히 배치) → "구글 지도"/"근처 백업 플랜" 버튼 → 표기 원칙. 백업 플랜은 같은 지역+같은 음식종류 중 실제 거리(위경도)가 가장 가까운 곳을 추천. "근처 디저트 맛집"(2026-08-18 추가, 로고와 동일한 진한 핑크 `#EE7191`)은 같은 지역의 디저트 카테고리 가게 중 가장 가까운 곳을 추천 — 가게 자신이 이미 디저트면 버튼이 숨겨지고, 근처에 디저트 가게가 없으면 "근처 디저트 없음"으로 대체 표시. 리뷰는 Google 실제 리뷰(최대 5개) + 네이버 블로그 리뷰(협찬 키워드 필터링 배지 포함, 2026-08-18 연동)를 랜덤으로 섞어서 병행 노출(2026-08-18 추가 — 원래 구글이 항상 먼저 나오던 걸 페이지 로드마다 다른 순서로 섞음). 리뷰 카드는 출처에 따라 클릭 시 이동하는 곳이 다름 — 구글 리뷰는 그 가게의 구글맵 페이지, 네이버 리뷰는 검색 API가 제공하는 블로그 원문 글 URL(`link`)로 각각 새 탭 이동. 리뷰 본문은 PC 3줄/모바일 5줄로 잘리고 "더보기"로 펼침. 리뷰 작성자 이름 중 한국 실명으로 추정되는 경우 일부 마스킹 처리(`유수아` → `유수○`) | ✅ 실데이터 |
...
- ~~핫페퍼 그루메 API 보강~~: 2026-08-18 완료 — `enrich-hotpepper`로 영업시간/정휴일/예산대/카드결제/URL을 보강(1,970곳 중 1,589곳 매칭), 상세페이지 사이드바에도 노출 완료
