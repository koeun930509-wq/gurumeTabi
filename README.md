...
수집 범위는 14개 지역(도쿄/오사카/후쿠오카/나고야/삿포로/오키나와/다카마쓰/마쓰야마/오카야마/히로시마/시즈오카/요나고/기타큐슈/나가사키) × 6개 카테고리(스시/라멘/우동/오코노미야키/돈카츠/타코야키)이며, 월 1회 정도 재수집을 권장합니다(자세한 절차는 `CLAUDE.md` 참고). 2026-08-18에 "근처 디저트 맛집" 기능을 위해 디저트 카테고리도 14개 지역 전체에 추가 수집(224곳, 네이버 리뷰 포함) — "카페"는 필터 목록엔 있지만 아직 미수집.
...
- 구글 소셜 로그인
- 스크랩(`scrapIds`)은 여전히 테이블만 있고 프론트는 localStorage 사용. 최근 검색어(`search_history`)는 2026-08-18에 실제 연동 완료(모바일 검색 오버레이에서 사용) — PC는 아직 UI가 없어 기록만 쌓이고 노출되지 않음
